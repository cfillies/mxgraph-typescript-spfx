import React from 'react';
// import PropTypes from 'prop-types';
// import { message, Tooltip } from 'antd';

import styles from './toolbar.module.scss';

// const userGuideText = 'group the cells selected';

// message.config({
//   top: 60,
//   duration: 2,
//   maxCount: 3,
// });
export interface IToolBarProps {
  editor: any;
  updateDiagramData: any;
}
export interface IToolBarState {
  editor: any;
  updateDiagramData: any;
}

export class Toolbar extends React.Component<IToolBarProps, IToolBarState> {
  constructor(props) {
    super(props);

    this.state = {
      editor: this.props.editor,
      updateDiagramData:() => { }

    };
  }

  public componentWillMount() {

  }
  private panning: boolean = false;
  public render(): React.ReactElement<IToolBarProps> {
    const {
      editor, updateDiagramData,
    } = this.props;

    const graph = editor && editor.graph;

    return (
      <div>
        <div key="toolbar" className={ styles.toolbar}>
          <div
            className={styles.toolbarbtn}
            onClick={() => {
              const diagramXml = window.localStorage.getItem('autosaveXml');

              updateDiagramData(diagramXml);
            }}
          >
            <img className={styles.icon} src="data:image/png;base64," alt="保存" />
            <span>save</span>
          </div>
          {/* <Tooltip placement="top" title={userGuideText}> */}
          <div
            className={styles.toolbarbtn}
            onClick={() => {
              const cellsSelected = graph.getSelectionCells();

              if (!(cellsSelected && cellsSelected.length > 1)) {
                return false;
              }

              let hasGroupCell = false;
              cellsSelected.forEach((cell) => {
                if (cell.isGroupCell || cell.isGrouped) {
                  hasGroupCell = true;
                }
              });

              if (hasGroupCell) {
                alert('can not group twice on the same cell');

                return false;
              }

              const allCells = graph.model.cells;

              let groupCount = 1;
              allCells && Object.keys(allCells).forEach((index) => {
                if (allCells[index].isGroupCell) {
                  groupCount += 1;
                }
              });

              const groupName = `group${groupCount}`;

              const { cellsGrouped } = editor.groupCells(cellsSelected, groupName);

              const nodeCodes: any[] = [];

              cellsGrouped.forEach((item) => {
                if (item.nodeCode) {
                  nodeCodes.push(item.nodeCode);
                }
              });

              return true;
            }}
          >
            <img className={styles.icon} alt="group" src="data:image/png;base64," />
            <span>group</span>
          </div>
          {/* </Tooltip> */}

          {/* <Tooltip placement="top" title="ungroup the cells in a group"> */}
          <div
            className={styles.toolbarbtn}
            onClick={() => {
              const cellsSelected = graph.getSelectionCells();

              if (!(cellsSelected && cellsSelected.length !== 0)) {
                return false;
              }

              editor.ungroupCells();

              return true;
            }}
          >
            <img className={styles.icon} alt="ungroup" src="data:image/png;base64," />
            <span>ungroup</span>
          </div>
          {/* </Tooltip> */}

          <div
            className={styles.toolbarbtn}
            onClick={() => {
              graph.zoomOut();
            }}
          >
            <img className={styles.icon} alt="zoom out" src="data:image/png;base64," />
            <span>zoom out</span>
          </div>
          <div
            className={styles.toolbarbtn}
            onClick={() => {
              graph.zoomIn();
            }}
          >
            <img className={styles.icon} alt="zoom in" src="data:image/png;base64," />
            <span>zoom in</span>
          </div>

          <div
            className={styles.toolbarbtn}
            onClick={() => {
              graph.zoomActual();
            }}
          >
            <img className={styles.icon} alt="zoom actual" src="data:image/png;base64," />
            <span>zoom actual</span>
          </div>
          <div
            className={styles.toolbarbtn}
            onClick={(e) => {
              e.currentTarget.classList.toggle('active');

              this.panning = !this.panning;

              if (this.panning) {
                editor.startPanning();
              } else {
                editor.stopPanning();
              }
            }}
          >
            <img className={styles.icon} alt="move" src="data:image/png;base64," />
            <span>move</span>
          </div>
        </div>,
      </div>
    );
  }
}


